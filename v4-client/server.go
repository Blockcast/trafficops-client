...
// GetServerCount retrieves server counts grouped by type and status from Traffic Ops.
// Optional query parameters: cdn, cachegroup, profile
func (to *Session) GetServerCount(opts RequestOptions) (tc.ServerCountResponse, toclientlib.ReqInf, error) {
	var data tc.ServerCountResponse
	reqInf, err := to.get(apiServers+"/count", opts, &data)
	return data, reqInf, err
}
