...
// ServerCount represents the count of servers for a specific status.
type ServerCount struct {
	Status string `json:"status"`
	Count  uint64 `json:"count"`
}

// ServerCountResponse is the format of a response to a GET request to
// /servers/count. The response maps server types to arrays of status counts.
type ServerCountResponse struct {
	Response map[string][]ServerCount `json:"response"`
	Alerts
}
